...
export const generateSystemEnvFile = async () => {
  ...
  const jwtSecret =
    envMap.get("JWT_SECRET") || (await deriveEntropy("jwt_secret"));
  const repoId = getRepoHash(
    data.appsRepoUrl || envMap.get("APPS_REPO_URL") || DEFAULT_REPO_URL
  );
  ...
  envMap.set(
    "GIT_SSL_VERIFY",
    typeof data.gitSSLVerify === "boolean"
      ? String(data.gitSSLVerify)
      : envMap.get("GIT_SSL_VERIFY") || "true"
  );
...
export const copySystemFiles = async (envMap: Map<EnvKeys, string>) => {
  ...
  if (envMap.get("PERSIST_TRAEFIK_CONFIG") === "true") {
    logger.warn(
      "Skipping the copy of traefik files because persistTraefikConfig is set to true"
    );
  } else {
  ...
  try {
    ...
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "torrents", "complete"),
      {
        recursive: true,
      }
    );
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "torrents", "incomplete"),
      {
        recursive: true,
      }
    );
    ...
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "usenet", "complete"),
      {
        recursive: true,
      }
    );
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "usenet", "incomplete"),
      {
        recursive: true,
      }
    );
    ...
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "downloads", "watch"),
      {
        recursive: true,
      }
    );
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "downloads", "complete"),
      {
        recursive: true,
      }
    );
    await fs.promises.mkdir(
      path.join(DATA_DIR, "media", "downloads", "incomplete"),
      {
        recursive: true,
      }
    );
