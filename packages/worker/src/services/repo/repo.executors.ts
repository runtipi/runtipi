...
export class RepoExecutors {
  ...
  public cloneRepo = async (url: string) => {
    try {
      ...
      this.logger.info(
        `Executing: git config --global http.sslVerify ${gitSSLVerify}`
      );
      await execAsync(
        `git config --global http.sslVerify ${gitSSLVerify}`
      ).then(({ stderr }) => {
        if (stderr) {
      ...
      if (branch) {
        this.logger.info(
          `Cloning repo ${repoUrl} on branch ${branch} to ${repoPath}`
        );
        cloneCommand = `git clone ${
          getEnv().gitSSLVerify === "true" && "-c http.sslVerify=false"
        } -b ${branch} ${repoUrl} ${repoPath}`;
      } else {
  ...
  public pullRepo = async (repoUrl: string) => {
    try {
      ...
      this.logger.info(
        `Executing: git config --global http.sslVerify ${
          gitSSLVerify === "true"
        }`
      );
      await execAsync(
      ...
      this.logger.info(
        `Executing: git config --global --add safe.directory ${repoPath}`
      );
      await execAsync(
      ...
      this.logger.info(
        `Executing: git -C ${repoPath} config pull.rebase false`
      );
      await execAsync(`git -C ${repoPath} config pull.rebase false`).then(
        ({ stderr }) => {
          if (stderr) {
            this.logger.error(`stderr: ${stderr}`);
          }
      ...
      this.logger.info(
        `Executing: git -C ${repoPath} rev-parse --abbrev-ref HEAD`
      );
