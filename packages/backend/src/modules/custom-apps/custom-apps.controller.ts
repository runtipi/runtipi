...
export class CustomAppController {
  ...
  async updateCustomApp(
    @Param("urn") appUrn: string,
    @Body() body: UpdateCustomAppDto
  ) {
    return await this.customAppService.updateCustomApp(
      castAppUrn(appUrn),
      body.config
    );
  }
  ...
  async updateAppMetadata(
    ...
    return await this.customAppService.updateAppMetadata(
      castAppUrn(appUrn),
      body.data
    );
  }
