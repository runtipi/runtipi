...
import { AppFormBody, UninstallAppBody, UpdateAppBody, UpdateUserConfigBody, appFormSchema, updateUserConfigSchema } from './dto/app-lifecycle.dto';
...
export class AppLifecycleController {
  ...
  @Patch(':id/update-user-config')
  async updateUserConfig(@Param('id') id: string, @Body() body: UpdateUserConfigBody) {
    const form = updateUserConfigSchema.parse(body);

    return this.appLifecycleService.updateAppUserConfig({ appId: id, env: form.env, compose: form.compose });
  }
}
