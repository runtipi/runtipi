...
export const OAuthActions = () => {
  ...
  if (oauthProviders.providers.length === 0) {
    return <p className="text-muted fst-italic mt-1">No providers configured yet.</p>;
  }
            ...
            {isExpanded && (
              <div key={provider.id} className={clsx('mt-3', isLast && 'border-top pt-3')}>
                <dl className="row mb-3 small">
                  <dt className="col-sm-2 fw-normal">Client ID</dt>
                  <dd className="col-sm-10 mb-1">
                    <code>{provider.clientId}</code>
                  </dd>
                  <dt className="col-sm-2 fw-normal">Callback URL</dt>
                ...
                {providerSubs.length === 0 ? (
                  ...
                  <>
                    <p className="small mb-2">Trusted users — these accounts are allowed to sign in via {provider.name}.</p>
