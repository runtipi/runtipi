...
const DialogPortal = ({
  children,
  ...props
}: DialogPrimitive.DialogPortalProps & ModalProps) => (
  <DialogPrimitive.Portal {...props}>
    <div className="modal modal-sm d-block dimmed-background">
      <div
        className={clsx(
          `modal-dialog modal-dialog-centered modal-${props.size || "lg"}`,
          "zoom-in"
        )}
      >
...
const DialogContent = ({
  ...
  children,
  ...props
}: React.ComponentProps<typeof DialogPrimitive.Content> & ModalProps) => (
  <DialogPortal type={props.type} size={props.size}>
    <DialogOverlay />
    <DialogPrimitive.Content
      className={clsx("modal-content mt-1", className)}
      {...props}
    >
      {children}
