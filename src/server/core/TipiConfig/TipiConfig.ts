...
export class TipiConfigClass {
  ...
  private genConfig() {
    let envFile = '';
    try {
      envFile = fs.readFileSync('/app/.env').toString();
    } catch (e) {
  ...
  private getFileConfig() {
    ...
    if (this.fileConfigCache && now - this.cacheTime < this.cacheTimeout) {
      ...
      const rawFileConfig = readJsonFile(`${this.config.rootFolder}/state/settings.json`) || {};
  ...
  public async setConfig<T extends keyof typeof envSchema.shape>(key: T, value: z.infer<typeof envSchema>[T], writeFile = false) {
    ...
    if (writeFile) {
      const currentJsonConf = readJsonFile(`${this.config.rootFolder}/state/settings.json`) || {};
      ...
      await fs.promises.writeFile(`${this.config.rootFolder}/state/settings.json`, JSON.stringify(parsed));
    }
  }
  ...
  public async setSettings(settings: TipiSettingsType) {
    ...
    await fs.promises.writeFile(`${this.config.rootFolder}/state/settings.json`, JSON.stringify(parsed.data));
