...
export const AppStoreTile: React.FC<{ app: App }> = ({ app }) => {
  ...
  return (
    <Link
      aria-label={app.name}
      className={clsx(styles.appTile)}
      href={`/app-store/${app.id}`}
      passHref
    >
      <div
        key={app.id}
        className="d-flex overflow-hidden align-items-center py-2 ps-2"
      >
        ...
        <div className="card-body">
          ...
          >
            ...
            {isNew ? (
              <div className="text-white badge me-1 bg-green">
                {t("APP_NEW")}
              </div>
            ) : null}
            {app.repo !== "runtipi-default" ? (
              <div className="text-white badge me-1 bg-azure-lt">
                {app.repo}
              </div>
            ) : null}
          </div>
          ...
          {app.categories?.map((category) => (
            ...
            >
              {t(
                `APP_CATEGORY_${category.toUpperCase() as Uppercase<typeof category>}`
              )}
            </div>
          ))}
        </div>
